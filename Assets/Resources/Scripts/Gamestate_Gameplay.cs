...
//		}
	...
	void Start () 
	{
		//PlayerPrefs.SetInt (PlayerPrefHandler.keyCoin, 500);
...
//		AndroidNotificationManager.instance.OnNotificationIdLoaded += OnNotificationIdLoaded;
	...
	public void StartGame()
	{
		...
		if (gsMode == Gamestate_Mode.Gameplay) 
		{
			...
			GameManager.GEMUCOINS = 0;
			if (PlayerPrefs.HasKey (PlayerPrefHandler.keyCoin))
				GameManager.GEMUCOINS = PlayerPrefs.GetInt (PlayerPrefHandler.keyCoin);
			else
					...
					if ( grabState == GrabState.None )
					{
						...
						else
						{
							...
							if (TimerHandler.instance.FreeGiftAvailablilty ()) {
								...
								ShowDialogBox("No Energy", "Get Free Energy by watching ads",false,"FreeEnergy",this.gameObject);
							} else {
	...
	public void OnDropSuccess(GameObject droppedob)
	{
		...
		if ( con )
		{
			...
			if ( droppedob.name.Contains("ticket") )
			{
				...
				guiResult.Show(con.icon,con.exp+" "+con.theName);
...
//				guiIngame.RefreshExpInfo();
				...
				guiResult.Show(con.icon,con.theName);

#region free coins and banner ads
